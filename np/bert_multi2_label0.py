...
def main():
    num_classes = 38  # クラス数（２値分類なら２）
    batch_size = 32
    epochs = 5   # 十分に学習できていれば、3くらいに減らしてもよい
    MAX = 80     # 80以上にするには 8GB以上のVRAMが必要

    train_labels, train_texts = get_tuples_label_sentence("train_label0.list")
    ...
    # 予測
    sent_id, test_texts = get_sentence("test_label0.list")
    x_test = to_features(test_texts, max_length)
    y_predicts = model.predict(x_test)

    # create output
    fp = open("output_label0", mode='w')
